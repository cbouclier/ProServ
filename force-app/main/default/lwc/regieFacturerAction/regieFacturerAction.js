...
import invoiceOrder from '@salesforce/apex/RegieBillingController.invoiceOrder';
import linkInvoicesToOrder from '@salesforce/apex/RegieBillingController.linkInvoicesToOrder';

export default class RegieFacturerAction extends LightningElement {
    @api recordId;

    @api async invoke() {
        try {
            const res = await invoiceOrder({ orderId: this.recordId });
            await this.pollLink();
